...
class CsvImportHandler
{
    ...
    public function importFromCsvFile($file)
    {
        ...
        foreach ($options as $productId => $productOptions) {	
          ...
          $write->query("DELETE FROM `{$write->getTableName('catalog_product_option')}` WHERE `product_id` = {$productId}");	
          ...
          $write->query("UPDATE `{$write->getTableName('catalog_product_entity')}` SET `has_options`=1, `required_options`={$required} WHERE `entity_id` = {$productId}");	

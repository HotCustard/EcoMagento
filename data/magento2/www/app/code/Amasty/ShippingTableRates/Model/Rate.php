				...
				if($totals['not_free_weight'] > 25){
					...
					if(!empty($items)){
						...
						foreach($items as $item) {
							...
							//echo '<br>product_id : '.$product_id.' - '.$productType.' - item_qty : '.$item_qty;
							if($productType=='simple'){
							
								$pallet = $product->getPallet();
								$pallet = $pallet / 100;
								if($pallet){
									if($configQty){
										$total_pallet = $total_pallet + ( $pallet * $configQty );
									}else{
										$total_pallet = $total_pallet + ( $pallet * $item_qty );
									}
								}
								$configQty  = 0;
								//echo '<br>t-pallet : '.$total_pallet;
							}else{
								$configQty = $item->getQty();
							}						
						}
						 
						$total_pallet = ceil($total_pallet);					
						$cost = $cost * $total_pallet;
					}else{

...
class MassUpdate extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        try {
            ...
            if ($status == \Webkul\Quotesystem\Model\Quotes::STATUS_SOLD) {
        ...
        if (count($quoteIds)) {
            foreach ($quoteIds as $key => $quoteId) {
                ...
                if ($quote->getStatus() == \Webkul\Quotesystem\Model\Quotes::STATUS_SOLD) {
                    ...
                    unset($quoteIds[$key]);
                } elseif ($status==\Webkul\Quotesystem\Model\Quotes::STATUS_UNAPPROVED
                    && ($quote->getStatus()==\Webkul\Quotesystem\Model\Quotes::STATUS_DECLINE
                    || $quote->getStatus()==\Webkul\Quotesystem\Model\Quotes::STATUS_APPROVED)
                ) {
                    $error[] = __('Can not update status for quote id %1', $quoteId);
                    unset($quoteIds[$key]);
                } elseif ($status == \Webkul\Quotesystem\Model\Quotes::STATUS_APPROVED
                && $productQty < $quote->getQuoteQty() &&
                $status == \Webkul\Quotesystem\Model\Quotes::STATUS_APPROVED) {
                    unset($quoteIds[$key]);
